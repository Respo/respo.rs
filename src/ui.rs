use crate::respo::{css::*, CssBoxSizing, CssColor, CssDisplay, CssLineHeight, CssSize, CssTextAlign, CssVerticalAlign, RespoStyle};
...
static_style!(ui_expand, &[("$0", RespoStyle::default().insert("flex", "1".to_owned()))]);
...
static_style!(ui_font_code, &[("$0", RespoStyle::default().font_family(CODE_FONTS.to_owned()))]);

static_style!(ui_font_fancy, &[("$0", RespoStyle::default().font_family(FANCY_FONTS.to_owned()))]);
